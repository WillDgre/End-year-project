class GamesController < ApplicationController
    ...
    def index_of
        begin
            ...
            if user
                ...
                @games = InstantGamingGame.first(number)
                render json: {success: @games}, status: 200
            else
                render :json => {error: "Invalid Token."}, status: 400 
            end
    ...
    def get_specific_game
        begin
            ...
            user = User.find_by(authentication_token: request.headers["API"])
            if user
                @games = InstantGamingGame.where("name LIKE ?", "%#{params[:name]}%").where('price > ?', params[:min_price]).where('price < ?', params[:max_price])
                if @games.length == 0
